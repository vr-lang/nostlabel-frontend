...
export const VerifyOTP: React.FC = () => {
  ...
  // Retrieve email and registration details from router state
  const state = location.state as { email?: string; registrationDetails?: any } | null;
  const email = state?.email || '';
  const registrationDetails = state?.registrationDetails || null;

  // If no email exists in state, block view and redirect to register
  useEffect(() => {
    if (!email) {
      navigate('/register');
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (registrationDetails) {
        const regRes = await authService.customerRegister(registrationDetails);
        if (regRes.success && regRes.token && regRes.user) {
          // Store token & context session instantly
          setSession(regRes.token, regRes.user);
          setSuccessMsg('ACCOUNT VERIFIED & CREATED');
          
          setTimeout(() => {
            navigate('/');
          }, 1500);
        } else {
          setErrorMsg('ACCOUNT REGISTRATION FAILURE');
        }
      } else {
        setErrorMsg('REGISTRATION DETAILS TIMED OUT. PLEASE RE-REGISTER.');
      }
    } catch (err: any) {
